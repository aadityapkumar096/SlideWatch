...
const r = envReport();
console.log("ENV → JS:", mask(process.env.GOOGLE_MAPS_JS_KEY), "WEB:", mask(process.env.GOOGLE_MAPS_WEB_KEY), "OW:", r.OPENWEATHER_API_KEY, "AW:", r.ACCUWEATHER_API_KEY);
...
// Health
app.get("/api/health", (_req, res) => res.json({ ok: true, time: new Date().toISOString(), env: envReport() }));
...
// Tsunami
app.get("/api/tsunami", async (_req, res) => {
    try {
        const key = "tsunami";
        const cached = await cacheGet(key);
        if (cached)
...
// Storm tracks + cones
app.get("/api/stormOverlays", async (_req, res) => {
    try {
