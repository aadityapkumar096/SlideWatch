...
function owKey() {
    ...
    if (!k)
        throw new Error("OPENWEATHER_API_KEY is missing (set it in .env)");
    return k;
}
function awKey() {
...
function synthAlertsFromForecast(list) {
...
export async function openWeatherPack(lat, lon) {
    const key = owKey();
    ...
    if (alerts.length === 0)
        alerts = synthAlertsFromForecast(list);
    return { mm1h, mm24h, mm72h, forecast6h, humidity, alerts };
}
// ---- AccuWeather alerts (optional) ----
export async function accuWeatherAlerts(lat, lon) {
    const key = awKey();
    if (!key)
        return [];
    try {
