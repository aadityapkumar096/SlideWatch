...
export function slopeFromGrid(elevGrid, cellSizeMeters) {
    if (elevGrid.length < 3 || elevGrid[0].length < 3)
        throw new Error("Grid too small");
    const n = elevGrid.length, m = elevGrid[0].length;
    const i = Math.floor(n / 2), j = Math.floor(m / 2);
    const dzdx = (elevGrid[i - 1][j + 1] + 2 * elevGrid[i][j + 1] + elevGrid[i + 1][j + 1] -
        (elevGrid[i - 1][j - 1] + 2 * elevGrid[i][j - 1] + elevGrid[i + 1][j - 1])) /
        (8 * cellSizeMeters);
    const dzdy = (elevGrid[i + 1][j - 1] + 2 * elevGrid[i + 1][j] + elevGrid[i + 1][j + 1] -
...
export function calcARI(hydro) {
    const w1 = 0.5, w3 = 0.25, w24 = 0.15, w72 = 0.1, wf = 0.4;
    const obs = w1 * (hydro.mm1h || 0) + w3 * (hydro.mm3h || 0) + w24 * (hydro.mm24h || 0) + w72 * (hydro.mm72h || 0);
...
export function computeRisk(signals) {
    ...
    const recent = (quakes || [])
        .filter((q) => Date.now() - q.time < 24 * 3600e3)
...
export function quakeHazard(quakes) {
    if (!quakes?.length)
        return { hazard: 0, strongest: null };
