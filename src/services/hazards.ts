...
export function extractHazardsFromAlerts(alerts: any[] = []) {
  const norm = (s: any) => String(s || "").toUpperCase();
  const tags = alerts.map((a: any) =>
    norm(a.event || a.type || a.category || a.headline || a.title)
  );
...
export async function nhcCurrentStorms() {
  try {
    ...
    const storms = (data?.activeStorms || data || []).map((s: any) => ({
