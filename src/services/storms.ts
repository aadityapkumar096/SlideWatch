...
export async function fetchStormOverlays(): Promise<{ storms: StormOverlay[] }> {
  const entries = (await Promise.all(FEEDS.map(fetchAtom))).flat();
  // Group by storm id/name; fetch KMLs (track & cone)
  const byStorm = new Map<string, { name: string; basin: string; trackUrl?: string; coneUrl?: string }>();
  ...
  const storms: StormOverlay[] = [];
  for (const [id, info] of byStorm.entries()) {
    const track = info.trackUrl ? await parseTrackKml(info.trackUrl).catch<LatLon[]>(() => []) : [];
    const cones = info.coneUrl ? await parseConeKml(info.coneUrl).catch<LatLon[][]>(() => []) : [];
...
async function fetchAtom(feedUrl: string): Promise<{ title: string; link: string }[]> {
  try {
    ...
    return entries
      ...
      .filter((x: any) => x.link.endsWith(".kml"));
  } catch {
    return [];
  }
}
...
// KML helpers
function coordsToLatLon(coordStr: string): LatLon[] {
  return coordStr
    .trim()
    .split(/\s+/)
...
async function parseTrackKml(url: string): Promise<LatLon[]> {
...
async function parseConeKml(url: string): Promise<LatLon[][]> {
  const { data } = await axios.get(url, { timeout: 10000 });
  const xml = await parseStringPromise(data);
...
function findAll(obj: any, path: string[]): any[] {
  const out: any[] = [];
  function dfs(node: any, depth: number) {
    if (!node) return;
    if (depth === path.length) { out.push(node); return; }
    ...
    for (const k of Object.keys(node || {})) {
      const v = node[k];
      if (typeof v === "object") dfs(v, depth);
    }
  }
  dfs(obj, 0);
  return out;
