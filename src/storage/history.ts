...
const ins = db.prepare(
  `INSERT INTO rainfall (lat,lon,ts,mm1h,mm3h,mm24h,mm72h) VALUES (?,?,?,?,?,?,?)`
);

export function storeRain(lat: number, lon: number, tsMs: number, pack: any) {
  ins.run(lat, lon, tsMs, pack.mm1h || 0, pack.mm3h || 0, pack.mm24h || 0, pack.mm72h || 0);
}

export function computeEMI(lat: number, lon: number, nowMs = Date.now(), halfLifeDays = 3) {
  ...
  const rows = db
    ...
    .all(lat, lon) as Array<{ ts: number; mm1h: number; mm24h: number }>;
  let emi = 0;
  for (const r of rows) {
    const dt = nowMs - r.ts;
